...
**详述**（可编辑）
子代理调度规则用于管理子代理的审查与实现生命周期，尤其在 subagent-driven-development 方法中：审查子代理完成审查并返回结果后应立即关闭；实现子代理则应保持打开状态，直到其对应任务的审查通过；若审查未通过，修复循环会恢复该实现子代理继续修改。如果运行环境限制导致无法向已生成的代理发送后续消息（即无法恢复现有代理），则应将每个修复轮次作为全新的实现子代理进行调度，并提供简要说明、报告文件以及发现的问题，以维持开发流程。

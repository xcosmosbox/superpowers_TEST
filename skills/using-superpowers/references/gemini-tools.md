...
**详述**（可编辑）
在 Gemini CLI 中，指令文件指 `GEMINI.md`。其加载采用层次化机制：全局配置文件位于 `~/.gemini/GEMINI.md`，会被首先加载；项目级配置文件存在于当前工作区目录及其所有祖先目录中，均会被依次发现并加载；此外，当工具访问某个子目录中的文件时，该子目录下存在的 `GEMINI.md` 也会被加载。这种层次化方式确保在不同层级均可提供指令，且最终指令是各层级的组合。
...
**详述**（可编辑）
更新当前对话的主题或战略意图元数据信息，通过 Gemini CLI 中的 update_topic 工具实现。
...
**详述**（可编辑）
任务追踪概念是任务工件和任务追踪器工具集的核心概念，用于实现任务跟踪功能，涵盖任务的创建、状态更新、生命周期管理以及任务工件（如任务列表）和追踪器工具集的整体设计。
...
**详述**（可编辑）
计划模式控制工具提供了在Gemini CLI中进入和退出只读计划模式的能力。包含enter_plan_mode操作，用于切换到只读计划模式，不修改任何文件；以及exit_plan_mode操作，用于从只读计划模式退出，恢复正常操作状态。
